...
class Smart_Product_Emails_Output {
	...
	public function __construct() {
		...
		// ON-HOLD STATUS.
		add_action( 'woocommerce_order_status_pending_to_on-hold_notification', array( $this, 'status_action_onhold' ), 10 );
		add_action( 'woocommerce_order_status_pending_to_on-hold_notification', array( $this, 'smart_product_emails_insert_content' ), 10, 2 );
		add_action( 'woocommerce_order_status_failed_to_on-hold_notification', array( $this, 'status_action_onhold' ), 10 );
		add_action( 'woocommerce_order_status_failed_to_on-hold_notification', array( $this, 'smart_product_emails_insert_content' ), 10, 2 );
		add_action( 'woocommerce_order_status_cancelled_to_on-hold_notification', array( $this, 'status_action_onhold' ), 10 );
		add_action( 'woocommerce_order_status_cancelled_to_on-hold_notification', array( $this, 'smart_product_emails_insert_content' ), 10, 2 );
		...
		// COMPLETED STATUS.
		add_action( 'woocommerce_order_status_completed_notification', array( $this, 'status_action_completed' ), 10 );
		add_action( 'woocommerce_order_status_completed_notification', array( $this, 'smart_product_emails_insert_content' ), 10, 2 );
		...
		if ( ! function_exists( 'smart_product_emails_output_message' ) ) {
			...
			function smart_product_emails_output_message( $order, $sent_to_admin, $email, $shown_messages ) {
				...
				foreach ( $items as $item ) {
					...
					$spemail_id_onhold = (int) $product->get_meta('spemail_id_onhold');
					$spemail_location_onhold = $product->get_meta('location_onhold');
					...
					$spemail_id_completed = (int) $product->get_meta('spemail_id_completed');
					$spemail_location_completed = $product->get_meta('location_completed');
					...
					// If there is an email assigned to the Order Statuses...
					if ( ! empty( $spemail_id_onhold ) || ! empty( $spemail_id_processing ) || ! empty( $spemail_id_completed ) ) {

						// ON-HOLD Status.
						// --------------------------------
						if ( 'woocommerce_order_status_on-hold' === $this_order_status_action && !empty( $spemail_id_onhold ) ) {

							// If there is an email assigned for 'On hold' status and this message is not already shown, 
							// AND if the message location set for the 'On hold' message is the current email template location...
							if ( !in_array( $spemail_id_onhold, $shown_messages, true ) && 
                    			$spemail_location_onhold === $this_email_template_location ) {
								...
								$output .= wp_kses_post( nl2br( get_post_field( 'post_content', $spemail_id_onhold ) ) );
								...
								$shown_messages[] = $spemail_id_onhold;
							}
						}
						...
						// COMPLETED Status.
						// --------------------------------
						if ( 'woocommerce_order_status_completed' === $this_order_status_action && !empty( $spemail_id_completed ) ) {

							// If there is an email assigned for 'Completed' status and this message is not already shown, 
							// AND if the message location set for the 'Completed' message is the current email template location...
							if ( !in_array( $spemail_id_completed, $shown_messages, true ) && 
								$spemail_location_completed === $this_email_template_location ) {
								...
								// Output the_content of the saved SPE Message ID.
								$output .= wp_kses_post( nl2br( get_post_field( 'post_content', $spemail_id_completed ) ) );
								...
								// Update 'shown_emails' var.
								$shown_messages[] = $spemail_id_completed;
