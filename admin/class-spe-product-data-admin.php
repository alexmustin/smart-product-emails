...
class SmartProductEmails_Product_Data_Admin {
	...
	public function add_smart_product_emails_tab( $original_prodata_tabs ) {
		...
		$new_custom_tab['smart-product-emails'] = array(
			'label'  => __( 'Smart Product Emails', 'smart-product-emails' ),
			...
			<div class='options_group'>
				...
				<div class="spe-product-options-table">
					...
					<div class="table-header">
						<div class="status">
							<?php echo esc_html__( 'Order Status', 'smart-product-emails' ); ?>
							<span class="woocommerce-help-tip" data-tip="<?php echo esc_html__( 'The Order Status Email which is automatically generated by WooCommerce.', 'smart-product-emails' ); ?>"></span>
						</div>
						<div class="message">
							<?php echo esc_html__( 'SPE Message', 'smart-product-emails' ); ?>
							<span class="woocommerce-help-tip" data-tip="<?php echo esc_html__( 'Select a SPE Message to be displayed in the Order Status email.', 'smart-product-emails' ); ?>"></span>
						</div>
						<div class="location">
							<?php echo esc_html__( 'Content Location', 'smart-product-emails' ); ?>
							<span class="woocommerce-help-tip" data-tip="<?php echo esc_html__( 'Select where your custom content will display in the Order Status email.', 'smart-product-emails' ); ?>"></span>
						</div>
					</div>
					...
					function smartproductemails_get_message_title( $msg_id = 0, $status_name = '' ) {
						...
						if ( get_post_status( $msg_id ) !== 'publish' ) {
							...
							$spemail_saved_text = '(' . __( 'Invalid SPE Message Selected', 'smart-product-emails' ) . ')';
						} else {
							// Saved SPE Mail is published.
							$spemail_saved_text = $spemail_title . '';
						}

						$default_search_msg = __( 'Search SPE Messages...', 'smart-product-emails' );
					...
					function smartproductemails_show_saved_message( $status_name = '' ) {
						...
						<div class="form-field spemail_search_field">
							...
							if ( ! empty( $smartproductemails_message_id_currentstatus ) ) {
								...
								?>
								<input class="spemail_search_field_input" type="text" name="spemail_search_<?php echo esc_attr( $status_name ); ?>" id="spemail_search_<?php echo esc_attr( $status_name ); ?>" placeholder="<?php echo esc_html__( 'Search SPE Messages...', 'smart-product-emails' ); ?>" autocomplete="off"></input>
								<?php
							}
							?>
							<div class="spemail_search_results hide" id="spemail_<?php echo esc_attr( $status_name ); ?>_search_results">
								<div class="spemail_search_results_wrap">
									<p class="placeholder"><?php echo esc_html__( 'Search results will appear here', 'smart-product-emails' ); ?></p>
								</div>
							</div>
						</div>
						<?php
					...
					function smartproductemails_show_location_select( $status_name = '' ) {
						...
						$location_select_arr = array(
							...
							'woocommerce_email_before_order_table' => __( 'Before Order Table', 'smart-product-emails' ),
							'woocommerce_email_after_order_table' => __( 'After Order Table', 'smart-product-emails' ),
							'woocommerce_email_order_meta' => __( 'After Order Meta', 'smart-product-emails' ),
							'woocommerce_email_customer_details' => __( 'After Customer Details', 'smart-product-emails' ),
							'woocommerce_email_footer' => __( 'Email Footer', 'smart-product-emails' ),
						);
					...
					<div class="table-row">
						<div class="status order_status">
							<mark class="order-status status-processing">
								<span><?php echo esc_html__( 'Processing', 'smart-product-emails' ); ?></span>
							</mark>
						</div>
	...
	public function ajax_spe_fetch_script() {
		...
		const speI18n = {
			edit: <?php echo wp_json_encode( __( 'Edit', 'smart-product-emails' ) ); ?>,
			remove: <?php echo wp_json_encode( __( 'Remove', 'smart-product-emails' ) ); ?>
		};
		<?php
		/**
			...
			$(document).on('click', function(e) {
				...
				if ( targetClass == 'spemail_search_field_input' ) {
					...
					if (!$btnContainer.hasClass('remove')) {
						...
						var editBtn = '<a href="<?php echo esc_url( admin_url('post.php?post=') ); ?>' + theid + '&action=edit" target="_blank" class="button edit-spemail" title="' + speI18n.edit + '"><span class="dashicons dashicons-edit"></span></a>';
	...
	public function spe_data_fetch() {
		...
		try {
			...
			if ( ! current_user_can( 'edit_products' ) ) {
				...
				wp_die( esc_html__( 'You do not have permission to perform this action.', 'smart-product-emails' ), 403 );
			}
			...
			if ( ! isset( $_POST['ajax_nonce'] ) || ! wp_verify_nonce( sanitize_key( $_POST['ajax_nonce'] ), 'ajax_nonce_action' ) ) {
				...
				wp_die( esc_html__( 'Security check failed.', 'smart-product-emails' ), 403 );
			}
			...
			if ( $the_query->have_posts() ) {
				...
				<p class="placeholder error">
					<?php
					echo esc_html__( 'Sorry! No posts match your search. Please add some ', 'smart-product-emails' );
					echo wp_kses( $add_messages_text, $allowed_tags );
					echo esc_html__( ' and try again.', 'smart-product-emails' );
					?>
				</p>
				<?php
			...
			?>
			<p class="placeholder error">
				<?php echo esc_html__( 'An error occurred while searching. Please try again.', 'smart-product-emails' ); ?>
			</p>
			<?php
			wp_die();
		}
