...
class SPE_Product_Data_Admin {
					...
					<div class="table-row">
						<div class="status order_status">
							<mark class="order-status status-on-hold">
								<span><?php echo esc_html__( 'On hold', 'smart_product_emails_domain' ); ?></span>
							</mark>
						</div>
						...
						<?php
						// Create a nonce.
						wp_nonce_field( 'status_onhold_action', 'spemail_onhold_nonce' );
						?>
					</div>
					...
					<div class="table-row">
						<div class="status order_status">
							<mark class="order-status status-completed">
								<span><?php echo esc_html__( 'Completed', 'smart_product_emails_domain' ); ?></span>
							</mark>
						</div>
						<label class="label-mobile"><?php echo esc_html__( 'Smart Product Email:', 'smart_product_emails_domain' ); ?></label>
						<div class="message">
						...
						<?php
						// Create a nonce.
						wp_nonce_field( 'status_completed_action', 'spemail_completed_nonce' );
						?>
					</div>
	...
	public function ajax_spe_fetch_script() {
		...
		function fetch_spe_posts_onhold(){
			jQuery.ajax({
				url: '<?php echo esc_url( admin_url( 'admin-ajax.php' ) ); ?>',
				type: 'post',
				data: {
					action: 'spe_data_fetch',
					keyword: jQuery('#spemail_search_onhold').val(),
					ajax_nonce: jQuery('#ajax_nonce').val(),
				},
				success: function(data) {
					jQuery('#spemail_onhold_search_results .spemail_search_results_wrap').html( data );
				}
			});
		}
		...
		function fetch_spe_posts_completed(){
			jQuery.ajax({
				url: '<?php echo esc_url( admin_url( 'admin-ajax.php' ) ); ?>',
				type: 'post',
				data: {
					action: 'spe_data_fetch',
					keyword: jQuery('#spemail_search_completed').val(),
					ajax_nonce: jQuery('#ajax_nonce').val(),
				},
				success: function(data) {
					jQuery('#spemail_completed_search_results .spemail_search_results_wrap').html( data );
				}
			});
		}
		...
		jQuery(document).ready( function($) {
			...
			$('#spemail_search_onhold').keyup(function () {
				fetch_spe_posts_onhold();
			});
			...
			$('#spemail_search_completed').keyup(function () {
				fetch_spe_posts_completed();
			});
	...
	public function save_smart_product_emails_tab_fields( $post_id ) {
		...
		// ON-HOLD.
		if (isset($_POST['spemail_id_onhold'], $_POST['spemail_onhold_nonce']) && 
			wp_verify_nonce(sanitize_key($_POST['spemail_onhold_nonce']), 'status_onhold_action')) {
			$msg_onhold_id = sanitize_text_field(wp_unslash($_POST['spemail_id_onhold']));
			$product->update_meta_data('spemail_id_onhold', $msg_onhold_id);
		}

		if (isset($_POST['onhold-location'], $_POST['spemail_onhold_nonce']) && 
			wp_verify_nonce(sanitize_key($_POST['spemail_onhold_nonce']), 'status_onhold_action')) {
			$msg_onhold_location = sanitize_text_field(wp_unslash($_POST['onhold-location']));
			$product->update_meta_data('location_onhold', $msg_onhold_location);
		}
		...
		// COMPLETED.
		if (isset($_POST['spemail_id_completed'], $_POST['spemail_completed_nonce']) && 
			wp_verify_nonce(sanitize_key($_POST['spemail_completed_nonce']), 'status_completed_action')) {
			$msg_completed_id = sanitize_text_field(wp_unslash($_POST['spemail_id_completed']));
			$product->update_meta_data('spemail_id_completed', $msg_completed_id);
		}
		
		if (isset($_POST['completed-location'], $_POST['spemail_completed_nonce']) && 
			wp_verify_nonce(sanitize_key($_POST['spemail_completed_nonce']), 'status_completed_action')) {
			$msg_completed_location = sanitize_text_field(wp_unslash($_POST['completed-location']));
			$product->update_meta_data('location_completed', $msg_completed_location);
		}


		// // Update the post meta.
		// update_post_meta( $post_id, 'spemail_id_onhold', $msg_onhold_id );
		// update_post_meta( $post_id, 'location_onhold', $msg_onhold_location );

		// update_post_meta( $post_id, 'spemail_id_processing', $msg_processing_id );
		// update_post_meta( $post_id, 'location_processing', $msg_processing_location );

		// update_post_meta( $post_id, 'spemail_id_completed', $msg_completed_id );
